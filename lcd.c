...
void HAL_LCD_SpiInit(void)
{
    ...
    EUSCI_B0->CTLW0 =   EUSCI_B_CTLW0_CKPH | // phase of 1
                        ...
                        //EUSCI_B_CTLW0_STEM | // UCxSTE digital output (not needed)
...
void Crystalfontz128x128_SetDrawFrame(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1)
{
    ...
    HAL_LCD_writeData((uint8_t)(x0 & 0xFF));
    HAL_LCD_writeData((uint8_t)(x1 >> 8));
    HAL_LCD_writeData((uint8_t)(x1 & 0xFF));
    // ADD CODE
    // Write the CM_CASET command followed by the 4 bytes of data
    // used to set the Column start and end rows.
    HAL_LCD_writeCommand(CM_RASET);
    HAL_LCD_writeData((uint8_t)(y0 >> 8));
    HAL_LCD_writeData((uint8_t)(y0 & 0xFF));
    HAL_LCD_writeData((uint8_t)(y1 >> 8));
    HAL_LCD_writeData((uint8_t)(y1 & 0xFF));
}
